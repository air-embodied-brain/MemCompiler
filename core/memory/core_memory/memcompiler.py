...
class Memcompiler(MASMemoryBase):
    """
    Memcompiler: Hierarchical Memory for Multi-Agent Systems
    A three-tier hierarchical graph structure composed of the Insight Graph, Query Graph, and Interaction Graph.
    ...
    def retrieve_memory(
        ...
        **args
        ...
        for success_task in successful_task_trajectories:
            prompt: str = MemcompilerPrompts.generative_task_user_prompt.format(
                trajectory=success_task.task_description + '\n' + success_task.task_trajectory,
                query_scenario=query_task
            )
            response: str = self.llm_model(messages=[Message('system', MemcompilerPrompts.generative_task_system_prompt), 
                                                     Message('user', prompt)])
    ...
    def _extract_mas_message(self, mas_message: MASMessage) -> MASMessage:
        ...
        system_prompt = MemcompilerPrompts.extract_true_traj_system_prompt
        prompt_template = MemcompilerPrompts.extract_true_traj_user_prompt
    ...
    def _detect_mistakes(self, mas_message: MASMessage) -> str:
        user_prompt: str = MemcompilerPrompts.detect_mistakes_user_prompt.format(task=mas_message.task_description, trajectory=mas_message.get_extra_field('clean_traj'))
        messages: list[Message] = [Message('system', MemcompilerPrompts.detect_mistakes_system_prompt), 
    ...
    def project_insights(self, raw_insights: list[str], role: str = None, task_traj: str = None) -> list[str]:
        ...
        raw_insights_str = '\n'.join(raw_insights)
        if not task_traj:
            system_prompt = MemcompilerPrompts.project_insights_system_prompt
            user_prompt: str = MemcompilerPrompts.project_insights_user_prompt.format(
                role=role,
                insights=raw_insights_str
            )
        else:
            system_prompt = MemcompilerPrompts.project_insights_with_traj_system_prompt
            user_prompt: str = MemcompilerPrompts.project_insights_with_traj_user_prompt.format(
                role=role,
                insights=raw_insights_str,
...
class TaskLayer:
    ...
    def cluster_tasks(self) -> None:
        ...
        try:
            # FINCH returns (cluster_labels, num_clusters, req_c)
            # We only need the finest partition (first column of cluster_labels)
            c, num_clust, req_c = FINCH(X, distance='cosine', verbose=False)
            labels = c[:, 0]  # Use the finest partition
...
class InsightsManager:
    ...
    def __post_init__(self):
        ...
        logger_name = f"{__name__}.{self.namespace}.{self.working_dir.replace(os.sep, '.') }"
    ...
    def merge_insights(self) -> None:
        ...
        total_merged = sum(len(rules) for rules in merged_label_rules.values())
        if total_merged == 0:
            self.logger.info('[merge_insights] All merged results are empty, skipping clear to preserve existing insights.')
            return
    ...
    def _merge_rules(self, rules: list[str]) -> list[str]:
        ...
        for i in range(0, len(rules), batch_size):
            ...
            limited_number: int = max(actual_num // 3, 1)

            user_prompt = MemcompilerPrompts.merge_rules_user_prompt.format(
                current_rules='\n'.join(batch),
                limited_number=limited_number
            )
            messages = [Message('system', MemcompilerPrompts.merge_rules_system_prompt),
                        Message('user', user_prompt)]
    ...
    def _finetune_insights(
        ...
        suffix: str = MemcompilerPrompts.finetune_insights_suffix['full'] if len(self.insights_memory) > MAX_RULE_THRESHOLD \
                      else MemcompilerPrompts.finetune_insights_suffix['not_full']
    ...
    def _build_comparative_prompts(self, true_traj: MASMessage, false_traj: MASMessage, insights: list[dict]) -> list[Message]:
        ...
        prompt = MemcompilerPrompts.critique_compare_rules_user_prompt.format(   
        ...
        return [Message(role='system', content= MemcompilerPrompts.critique_compare_rules_system_prompt), Message(role='user', content=prompt)] 
    ...
    def _build_success_prompts(
        ...
        prompt = MemcompilerPrompts.critique_success_rules_user_prompt.format(
            success_history='\n'.join(history),
            existing_rules=rule_text
        )

        return [Message(role='system', content=MemcompilerPrompts.critique_success_rules_system_prompt), Message(role='user', content=prompt)]
